...
if (isset($_GET['order_type']) && $_GET['order_type'] == 1) {
    ...
    $insert_vnpay = "INSERT INTO vnpay(
        vnp_amount, vnp_bankcode, vnp_banktranno, vnp_cardtype, vnp_orderinfo, vnp_paydate, vnp_tmncode, vnp_transactionno, order_code, payment_status)
        VALUE('" . $vnp_Amount . "','" . $vnp_BankCode . "','" . $vnp_BankTranNo . "','" . $vnp_CardType . "','" . $vnp_OrderInfo . "','" . $vnp_PayDate . "','" . $vnp_TmnCode . "','" . $vnp_TransactionNo . "', '" . $order_code . "', '".$status."')";
    $query_vnpay = mysqli_query($mysqli, $insert_vnpay);

} else if (isset($_GET['partnerCode']) && isset($_GET['message']) && strval($_GET['message']) == 'Successful.') {
    $status == 1;
    ...
    $insert_momo = "INSERT INTO momo(
        partner_code, order_code, momo_amount, order_info, order_type, trans_id, payment_date, pay_type, payment_status)
        VALUE('" . $partnerCode . "','" . $orderId . "','" . $amount . "','" . $orderInfo . "','" . $orderType . "','" . $transId . "','" . $order_date . "','" . $payType . "', '".$status."')";
